...
**`mint recreate`** — Replaces the instance and root volume while preserving project data and user config. Orchestration sequence:
1. Stop instance
2. Detach project EBS volume
3. Terminate instance (destroys root EBS)
4. Launch new instance in the **same AZ** (required because EBS is AZ-scoped)
5. Attach project EBS to new instance
6. Mount project EBS at project directory
7. EFS mounts via fstab during boot
8. Bootstrap runs on new root volume
9. Health check validates all components; tags instance `mint:bootstrap=complete` on success
