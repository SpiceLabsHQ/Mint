...
2. **Local files** (config, keys) — fast, user-owned, no network dependency.
...
**Tier 1 — AWS is the source of truth for resource state.** Instance existence, status, IPs, tags, security groups, and key pairs are always queried live from AWS APIs. Nothing about resource state is cached locally. If AWS says a VM is running, it's running. If a tag is missing, the resource is not Mint-managed. This eliminates state drift, stale caches, and the "works on my machine" class of bugs.

**Tier 2 — Local files store user preferences and secrets.** Configuration (`~/.config/mint/config.toml`) holds user preferences: region, default instance type, default volume size, idle timeout. SSH private keys are stored locally. These are user-owned, machine-specific, and do not need to be shared or synchronized. They represent what the user *wants*, not what *exists*.
...
- **Local config is not backed up.** If a user loses their machine, they lose their config and SSH keys. Config is trivially recreated; SSH keys require `mint init` to regenerate and re-register. Acceptable for a dev tool.
