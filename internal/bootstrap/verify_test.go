...
func TestScriptContent(t *testing.T) {
	...
	requiredElements := []struct {
		...
		{"EFS home .ssh setup", "mkdir -p /home/ubuntu/.ssh"},
		{"EFS home projects symlink", "ln -sfn /mint/projects /home/ubuntu/projects"},
		{"user-bootstrap runs as ubuntu", "sudo -u ubuntu -H"},
		{"user-bootstrap git insteadOf scoped via env", "GIT_CONFIG_COUNT=3"},
